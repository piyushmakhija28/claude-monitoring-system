...
def auto_save_session(project_name):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # 🚨 AUTO-COMMIT INTEGRATION: After session save, commit all repos with changes
            log_daemon("AUTO-COMMIT", "Triggering auto-commit for all repos with changes")
            trigger_auto_commit()
...
def trigger_auto_commit():
    """Trigger auto-commit enforcer for all repos with changes"""
    try:
        log_daemon("AUTO-COMMIT", "Running auto-commit enforcer")

        result = subprocess.run(
            [
                "python",
                os.path.expanduser("~/.claude/memory/auto-commit-enforcer.py"),
                "--enforce-now"
            ],
            capture_output=True,
            text=True,
        ...
        if result.returncode == 0:
            log_daemon("AUTO-COMMIT", "Auto-commit completed successfully")
            # Log output for debugging
            if result.stdout:
                log_daemon("AUTO-COMMIT-OUTPUT", result.stdout.strip()[:200])  # First 200 chars
        else:
            log_daemon("AUTO-COMMIT-ERROR", f"Auto-commit failed: {result.stderr}")

    except subprocess.TimeoutExpired:
        log_daemon("AUTO-COMMIT-TIMEOUT", "Auto-commit timed out after 120 seconds")
    except Exception as e:
        log_daemon("AUTO-COMMIT-ERROR", f"Failed to trigger auto-commit: {e}")
