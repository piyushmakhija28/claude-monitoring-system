...
def analyze_projects():
    ...
    print(f"🔍 Analyzing {len(projects)} projects for patterns...")
    print()
    ...
    print(f"✓ Analyzed {len(project_data)} projects with content")
    print()
    ...
    for key, data in pattern_counts.items():
        if len(data['projects']) >= threshold:
            ...
            print(f"✅ Pattern detected: {item.upper()} ({category})")
...
def show_patterns():
    ...
    if not patterns_data['patterns']:
        print("📭 No patterns detected yet. Run analysis first:")
...
def suggest_patterns(topic):
    ...
    if not matches:
        print(f"❌ No patterns found related to: {topic}")
        print("\n💡 Try these topics:")
