...
def show_resume_prompt(project):
    ...
    print("🔍 Checking for incomplete work...")
    print()
    ...
    if summary_incomplete:
        print("📋 I found incomplete work in your project summary:")
        print()
        ...
        if items:
            ...
            if unique_items:
                print("  🔖 Incomplete markers detected:")
                for item in unique_items:
                    # Clean up the item
                    item_clean = item.replace('⏸️', '').replace('🔄', '').strip()
    ...
    if session_incomplete:
        ...
        print(f"📝 Last session ({age_str}) had incomplete items:")
        print()
