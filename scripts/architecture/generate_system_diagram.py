...
def zone(x, y, w, h, label, fill, stroke, font_style=1):
    style = ("swimlane;startSize=28;fontStyle=%d;fontSize=12;fillColor=%s;"
...
def diamond(x, y, w, h, label, fill="#FFFFFF", stroke="#000000"):
    style = ("rhombus;html=1;whiteSpace=wrap;fontSize=10;"
             "fillColor=%s;strokeColor=%s;strokeWidth=2;perimeter=rhombusPerimeter;") % (fill, stroke)
    cid = _id()
    cell = box(x, y, w, h, label, style)
    return cid, cell

def terminal(x, y, label, fill="#000000", stroke="#000000"):
    style = ("ellipse;html=1;fillColor=%s;strokeColor=%s;fontColor=#FFFFFF;"
             "fontStyle=1;fontSize=10;") % (fill, stroke)
    cid = _id()
    cell = box(x, y, 120, 40, label, style)
    return cid, cell

def ext_system(x, y, label, fill="#F5F5F5", stroke="#666666"):
    style = ("rounded=1;whiteSpace=wrap;html=1;fontSize=10;fontStyle=1;"
             "fillColor=%s;strokeColor=%s;strokeWidth=1;") % (fill, stroke)
    cid = _id()
    cell = box(x, y, 155, 44, label, style)
    return cid, cell

def section_label(x, y, w, h, label, color="#555555"):
    style = ("text;html=1;align=center;fontStyle=3;fontSize=11;"
             "fontColor=%s;fillColor=none;strokeColor=none;") % color
    return box(x, y, w, h, label, style)

def note(x, y, w, h, label):
    style = ("shape=mxgraph.general.note;html=1;whiteSpace=wrap;fontSize=9;"
...
DASHED_ARROW = "endArrow=open;endSize=8;html=1;strokeColor=#999999;strokeWidth=1;dashed=1;edgeStyle=orthogonalEdgeStyle;"
...
def build():
    ...
    # TITLE
    # ------------------------------------------------------------------
    C.append(box(10, 8, 1660, 44,
        "Claude Workflow Engine  \u2014  Complete System Architecture  (v1.5.0)",
        "text;html=1;align=center;fontStyle=1;fontSize=16;"
        "fillColor=#1e3a5f;fontColor=#FFFFFF;strokeColor=none;"))
    ...
    C.append(box(RX - 12, 62, R_W + 28, 1680,
    ...
    def rsys(y, label, fill, stroke):
        cid = _id()
        C.append(box(RX, y, R_W, 42, label,
    ...
    sq_id  = rsys(380,  "SonarQube  [ENABLE_SONARQUBE=1]\nQuality Gate / Scan",     "#F8CECC","#B85450")
    ...
    C.append(arrow_pts(MX+846, L1Y+34, MX+10, L1Y+28, "retry",
    ...
    # Step 7
    s7_id  = step(MX+10, cur_y, 280, 42,
        "Step 7  Final Prompt Generation\n[LLM \u2192 RAG]  3 files + [Figma tokens if ENABLE_FIGMA]", "#E8D5F5")
    ...
    # Step 8
    s8_id  = step(MX+10, cur_y, 260, 42,
        "Step 8  GitHub Issue + [Jira if ENABLE_JIRA]\n(cross-linked, issue_number stored)")
    ...
    C.append(box(MX+10, cur_y, MW-20, 28,
    ...
    C.append(arrow_pts(MX+870, hook_exit_y, RX, 1047+21, "",
    ...
    # Step 10
    s10_id = step(MX+10, cur_y, 280, 42,
        "Step 10  Implementation\n[CallGraph snapshot]  + [Jira \u2192 In Progress]  + [Figma started]", "#C9B3D4","#6A4080")
    ...
    # Step 11
    s11_id = step(MX+10, cur_y, 310, 42,
        "Step 11  PR + Code Review + Merge\n[GitHub] + [CallGraph diff] + [Jenkins CI] + [Figma review]", "#C9B3D4","#6A4080")
    ...
    # Step 13
    s13_id = step(MX+10, cur_y, 310, 42,
        "Step 13  Documentation Update\nCLAUDE.md / SRS / CHANGELOG + UML Mermaid + draw.io (12 types)", "#C9B3D4","#6A4080")
    ...
    # Step 14
    s14_id = step(MX+10, cur_y, 260, 42,
    ...
    # END terminal
    end_id = step(MX+10, cur_y, 120, 38,
        "END", "#1e3a5f", "#1e3a5f")
    ...
    # ---- LEGEND ----
    leg_y = L4Y + L4H + 10
    C.append(box(MX, leg_y, 800, 80, "LEGEND",
    ...
    for lx, ly, fill, stroke, lbl in legend_items:
        C.append(box(lx, ly, 18, 18, "",
            "rounded=0;fillColor=%s;strokeColor=%s;" % (fill, stroke)))
        C.append(box(lx+22, ly-2, 180, 22, lbl,
            "text;html=1;fontSize=9;align=left;fillColor=none;strokeColor=none;"))
