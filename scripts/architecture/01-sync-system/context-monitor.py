...
def _build_recommendation(zone: str, percentage: float, estimated_tokens: int) -> str:
    ...
    if zone == "GREEN":
        return (
            f"Context is OPTIMAL at {pct_str}. "
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Estimate Claude context window usage from session data."
    )
    parser.add_argument(
        "session_dir",
        nargs="?",
    ...
    if args.track:
        ...
        if args.json:
            ...
            for item in history:
                ...
                print(
                    f"  {name:<40} {item['percentage_display']:>8}  [{item['threshold_zone']}]"
                )
            print()
        return 0
