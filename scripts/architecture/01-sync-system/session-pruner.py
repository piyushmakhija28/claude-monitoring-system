...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Archive old Claude sessions per session-pruning-policy."
    )
    parser.add_argument(
        "sessions_dir",
        nargs="?",
    ...
    if args.sessions_dir:
        ...
        result = prune_sessions(
            target, args.max_age_days, args.keep_min, dry_run=args.dry_run
        )
        ...
        result = prune_all_projects(
            default_root, args.max_age_days, args.keep_min, dry_run=args.dry_run
        )
