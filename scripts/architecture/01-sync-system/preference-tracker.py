...
KNOWN_SKILLS = [
    "python-core", "python-design-patterns-core", "clean-architecture",
    "error-handling-patterns", "logging-patterns", "performance-optimization",
    "rdbms-core", "nosql-core", "redis-core", "message-queues-core",
    "api-design-core", "graphql-core", "authentication-core",
    "testing-core", "json-core", "system-design",
    "langgraph-core", "langchain-core", "fastmcp-core",
    "react-core", "angular-core", "swiftui-core",
    "docker", "kubernetes", "jenkins-pipeline",
]

# Known agent names to search for in session text
KNOWN_AGENTS = [
    "python-backend-engineer", "spring-boot-microservices",
    "ui-ux-designer", "angular-engineer", "swiftui-designer",
    "devops-engineer", "qa-testing-agent", "python-system-scripting",
    ...
    def _print_category(label: str, items: List[Dict[str, Any]]) -> None:
        ...
        if not items:
            ...
            for item in items:
                ...
                print(
                    f"    {item['name']:<35} "
                    f"[{bar}] {item['score'] * 100:.0f}%  "
                    f"({item['count']} sessions)"
                )
        print()
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Track user preferences from Claude session history."
    )
    parser.add_argument(
        "sessions_dir",
        nargs="?",
