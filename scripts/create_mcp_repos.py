...
SERVERS = [
    ...
    {
        "repo": "mcp-skill-manager",
        "file": "skill_manager_mcp_server.py",
        "description": "Complete skill and agent lifecycle management for Claude Code. Loads SKILL.md and agent.md files, extracts metadata (capabilities, triggers, dependencies), validates skills, ranks by relevance to current task, and detects conflicts between skills.",
        "tools": [
            ("skill_load_all", "Load all skills from ~/.claude/skills/ directory"),
            ("skill_load", "Load a single skill by name, returning full metadata"),
            ("skill_search", "Search skills by keyword across name, description, triggers"),
            ("skill_validate", "Validate a skill file for required fields and correct format"),
            ("skill_rank", "Rank loaded skills by relevance to a given task description"),
            ("skill_detect_conflicts", "Detect conflicting capabilities between active skills"),
            ("agent_load_all", "Load all agents from ~/.claude/agents/ directory"),
            ("agent_load", "Load a single agent by name, returning full metadata"),
        ],
        "env_vars": [
            ("CLAUDE_SKILLS_DIR", "Path to skills directory (default: ~/.claude/skills/)"),
            ("CLAUDE_AGENTS_DIR", "Path to agents directory (default: ~/.claude/agents/)"),
        ],
        "benefits": [
            "Programmatic skill discovery — no manual skill name lookup",
            "Relevance ranking surfaces the best skill for each task automatically",
            "Conflict detection prevents contradictory skills from being co-activated",
            "Metadata extraction enables skill composition and dependency graphs",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": False,
    },
    ...
    {
        "repo": "mcp-anthropic-provider",
        "file": "anthropic_mcp_server.py",
        "description": "Direct Anthropic Claude API access without SDK dependency. Provides model tier aliases (fast/balanced/deep), token cost estimation, streaming support, and health checking with API key validation. Uses stdlib urllib only.",
        "tools": [
            ("anthropic_generate", "Generate text using Claude (model, prompt, max_tokens, temperature)"),
            ("anthropic_health_check", "Verify API key and measure latency (cached 60s)"),
            ("anthropic_list_models", "List available Claude models with tier aliases"),
            ("anthropic_estimate_cost", "Estimate token cost before making a call"),
        ],
        "env_vars": [
            ("ANTHROPIC_API_KEY", "Anthropic API key (required)"),
            ("ANTHROPIC_DEFAULT_MODEL", "Default model (default: claude-sonnet-4-6)"),
            ("ANTHROPIC_MAX_TOKENS", "Default max tokens per call (default: 4096)"),
        ],
        "benefits": [
            "No anthropic SDK required — urllib only, minimal footprint",
            "Tier aliases (fast/balanced/deep) abstract model version churn",
            "Cost estimation before calling prevents budget surprises",
            "Health caching (60s) avoids repeated API key validation latency",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": False,
    },
    ...
    {
        "repo": "mcp-llm-provider",
        "file": "llm_mcp_server.py",
        "description": "Multi-provider LLM access wrapping the workflow engine provider chain. Supports Ollama (local GPU), Claude CLI, Anthropic API, and OpenAI with hybrid GPU-first routing, model discovery, step classification, and automatic fallback. Also generates git commit titles.",
        "tools": [
            ("llm_generate", "Generate text via configured provider chain with auto-fallback"),
            ("llm_list_models", "List available models across all providers"),
            ("llm_health_check", "Check health of all configured LLM providers"),
            ("llm_git_commit_title", "Generate a short git commit title from a diff/description"),
            ("llm_discover_models", "Discover locally available Ollama models"),
            ("llm_select_model", "Select optimal model for a given step/complexity"),
            ("llm_classify_step", "Classify pipeline step to determine model tier needed"),
            ("llm_hybrid_generate", "GPU-first hybrid: try Ollama, fall back to cloud on failure"),
        ],
        "env_vars": [
            ("ANTHROPIC_API_KEY", "Anthropic API key"),
            ("OPENAI_API_KEY", "OpenAI API key"),
            ("OLLAMA_ENDPOINT", "Ollama server URL (default: http://localhost:11434)"),
            ("LLM_DEFAULT_PROVIDER", "Default provider: ollama/anthropic/openai (default: ollama)"),
        ],
        "benefits": [
            "GPU-first routing: uses local Ollama when available, saves API costs",
            "Transparent fallback chain: no code changes needed when a provider is down",
            "Step classification enables cost-optimal model selection per pipeline step",
            "Git commit title generation from diffs reduces context needed from Claude",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": True,
        "engine_note": "Depends on scripts/langgraph_engine/llm_call.py from claude-workflow-engine",
    },
    {
        "repo": "mcp-llm-router",
        "file": "llm_router_mcp_server.py",
        "description": "Intelligent LLM routing and model selection without direct LLM invocation. Routes requests to Ollama (GPU-first for simple tasks) or cloud providers (Anthropic/OpenAI for complex tasks) based on step complexity classification. Decision-only: caller invokes the chosen provider.",
        "tools": [
            ("llm_route_request", "Route a request to optimal provider based on task complexity"),
            ("llm_classify_step", "Classify pipeline step (simple/medium/complex/critical)"),
            ("llm_select_model", "Select best model for complexity tier (fast/balanced/deep)"),
            ("llm_git_commit_title", "Generate git commit title without full LLM call"),
        ],
        "env_vars": [
            ("OLLAMA_ENDPOINT", "Ollama server URL (default: http://localhost:11434)"),
            ("LLM_COMPLEXITY_THRESHOLD", "Complexity score above which to use cloud (default: 0.6)"),
        ],
        "benefits": [
            "Decouples routing logic from invocation — router is stateless and fast",
            "GPU-first policy maximizes local compute usage, minimizes API spend",
            "Step classification is rule-based (no LLM needed), zero latency routing",
            "Works standalone or as part of the hybrid LLM provider chain",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": False,
    },
    {
        "repo": "mcp-ollama-provider",
        "file": "ollama_mcp_server.py",
        "description": "Pure Ollama local GPU inference provider. No fallback chain, no external APIs. Generates text, lists locally available models, checks server health (30s cache), discovers all models including multimodal, and can pull new models from Ollama registry.",
        "tools": [
            ("ollama_generate", "Generate text using a local Ollama model"),
            ("ollama_list_models", "List all locally downloaded models"),
            ("ollama_health_check", "Check Ollama server connectivity and response time (30s cache)"),
            ("ollama_discover_local_models", "Discover available models including multimodal capabilities"),
            ("ollama_pull_model", "Pull a model from Ollama registry (e.g., llama3.2, codellama)"),
        ],
        "env_vars": [
            ("OLLAMA_ENDPOINT", "Ollama server URL (default: http://localhost:11434)"),
        ...
        "benefits": [
            ...
            "Health caching (30s) avoids hammering Ollama on every tool call",
            "Model discovery includes multimodal flag for vision-capable models",
            "Pull tool enables automated model provisioning in setup scripts",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": False,
    },
    {
        "repo": "mcp-openai-provider",
        "file": "openai_mcp_server.py",
        "description": "Direct OpenAI GPT API access via stdlib urllib (no openai SDK). Supports model tier aliases (fast=gpt-4o-mini, balanced=gpt-4o, deep=gpt-4o), cost estimation, and health checking with 60-second response caching. ASCII-only Windows-safe.",
        "tools": [
            ("openai_generate", "Generate text using OpenAI GPT model (model, prompt, max_tokens)"),
            ("openai_health_check", "Verify API key and measure latency (cached 60s)"),
            ("openai_list_models", "List available models with tier aliases"),
            ("openai_estimate_cost", "Estimate token cost for a given prompt length"),
        ],
        "env_vars": [
            ("OPENAI_API_KEY", "OpenAI API key (required)"),
            ("OPENAI_DEFAULT_MODEL", "Default model (default: gpt-4o-mini)"),
            ("OPENAI_MAX_TOKENS", "Default max tokens per call (default: 4096)"),
            ("OPENAI_ORG_ID", "OpenAI organization ID (optional)"),
        ],
        "benefits": [
            "No openai SDK — pure urllib, minimal install footprint",
            "Tier aliases abstract GPT model version churn (fast/balanced/deep stay stable)",
            "Cost estimation prevents accidental large-prompt API overruns",
            "60s health cache means health checks don't count toward rate limits",
        ],
        "pip_deps": ["mcp", "fastmcp"],
        "engine_dep": False,
    },
