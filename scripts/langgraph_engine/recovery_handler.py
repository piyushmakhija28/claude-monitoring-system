...
def _is_transient_error(exc: Exception) -> bool:
    ...
    transient_keywords = (
        "timeout", "connection", "rate limit", "overloaded", "503", "502",
        "too many requests", "retry", "network", "unavailable", "refused",
    )
...
def _sigint_handler(signum: int, frame: Any) -> None:
    ...
    if _checkpoint_manager and _current_state:
        try:
            ...
            checkpoint_path = str(
                _checkpoint_manager.checkpoint_dir / f"step-{_current_step:02d}.json"
            )
    ...
    print(
        f"  python -m scripts.langgraph_engine.recovery_handler "
        f"resume {session_id} {checkpoint_id}",
        file=sys.stderr
    )
    print("=" * 60, file=sys.stderr)
...
class RecoveryHandler:
    ...
    def install_signal_handlers(self) -> None:
        ...
        if threading.current_thread() is not threading.main_thread():
            logger.warning(
                "[Recovery] install_signal_handlers() called outside main thread "
                "- SIGINT handler NOT installed"
            )
            return
    ...
    def resume_session(self, checkpoint_id: Optional[str] = None) -> bool:
        ...
        print(
            f"Resuming session {self.session_id} from step {next_step}...",
            file=sys.stderr
        )
        ...
        for step in range(next_step, 15):   # Steps 1-14
            ...
            for attempt in range(_MAX_STEP_RETRIES):
                try:
                    ...
                    self.error_logger.log_error(
                        ...
                        recovery_action=(
                            f"Retry {attempt + 1}/{_MAX_STEP_RETRIES} "
                            f"with {_backoff_delay(attempt):.0f}s backoff"
            ...
            if not step_success:
                logger.error(
                    f"[Recovery] Step {step} failed after {_MAX_STEP_RETRIES} attempts: "
                    f"{last_exc}"
                )
                return False
...
if __name__ == "__main__":
    """
    Usage:
        python -m scripts.langgraph_engine.recovery_handler resume <session_id>
        python -m scripts.langgraph_engine.recovery_handler resume <session_id> <checkpoint_id>
        python -m scripts.langgraph_engine.recovery_handler list   <session_id>
    """
    args = sys.argv[1:]
    ...
    if command == "resume":
        ...
        if not checkpoints:
            ...
            for cp in checkpoints:
                ...
                print(
                    f"  Step {cp['step']:02d}  {status:<6}  {cp['timestamp']}  "
                    f"id={cp['checkpoint_id']}{err_suffix}"
                )
