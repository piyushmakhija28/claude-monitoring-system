...
def validate_toon(toon: dict) -> tuple:
    ...
    if "timestamp" in toon:
        if not isinstance(toon["timestamp"], str) or not toon["timestamp"].strip():
            ...
            try:
                ...
                errors.append("timestamp must be a valid ISO-8601 string, got: " + toon["timestamp"])
    ...
    if "version" in toon:
        if not isinstance(toon["version"], str):
            ...
            errors.append("version must be '" + TOON_VERSION + "', got: " + str(toon["version"]))
    ...
    if "complexity_score" in toon:
        ...
        if not isinstance(score, int) or isinstance(score, bool):
            ...
            errors.append("complexity_score must be between 1 and 10, got: " + str(score))

    # files_loaded_count: non-negative int
    if "files_loaded_count" in toon:
        count = toon["files_loaded_count"]
        if not isinstance(count, int) or isinstance(count, bool):
            errors.append("files_loaded_count must be an int, got: " + type(count).__name__)
        elif count < 0:
            errors.append("files_loaded_count must be >= 0, got: " + str(count))
