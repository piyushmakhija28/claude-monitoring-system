...
class StandardsSchema:
    ...
    def validate(self, standard_dict: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for field, expected_type in self.FIELD_TYPES.items():
            if field in standard_dict:
                ...
                if not isinstance(value, expected_type):
                    errors.append(f"Field '{field}' must be {expected_type.__name__}, " f"got {type(value).__name__}")
        ...
        if isinstance(project_type, str) and project_type not in VALID_PROJECT_TYPES:
            errors.append(f"'project_type' must be one of {sorted(VALID_PROJECT_TYPES)}, " f"got '{project_type}'")
            ...
            for sub_key, sub_value in rules.items():
                if sub_key in RULES_SUBKEYS:
                    ...
                    if not isinstance(sub_value, expected):
                        errors.append(
                            f"rules.{sub_key} must be {expected.__name__}, " f"got {type(sub_value).__name__}"
                        )
        ...
        if isinstance(version, str):
            if not re.match(r"^\d+\.\d+\.\d+$", version):
                errors.append(f"'version' should follow semver (X.Y.Z), got '{version}'")
