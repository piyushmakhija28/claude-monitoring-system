        ...
        try:
            ...
            return {
                ...
                "by_level": {
                    ...
                    "standards_system": len(
                        [s for s in self.standards.values() if "02-standards" in s.get("level", "")]
                    ),
                    "execution_system": len(
                        [s for s in self.standards.values() if "03-execution" in s.get("level", "")]
                    ),
                    "other": len(
                        [
                            s
                            for s in self.standards.values()
                            if not any(x in s.get("level", "") for x in ["01-sync", "02-standards", "03-execution"])
                        ]
                    ),
                },
