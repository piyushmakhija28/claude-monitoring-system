        ...
        for neighbor in dependency_graph.get(node, []):
            if neighbor not in visited:
                ...
                if not any(frozenset(c) == cycle_key for c in cycles):
                    cycles.append(cycle)
                    logger.warning(
                        f"[DependencyResolver] Circular dependency detected: "
                        f"{' -> '.join(cycle)}"
                    )
...
def resolve_dependencies(
    ...
    while queue:
        ...
        if content is None:
            logger.warning(
                f"[DependencyResolver] Skill '{skill_name}' not found in skill_contents "
                f"(required by '{parent}')"
            )
            unresolvable.append(skill_name)
        ...
        logger.debug(
            f"[DependencyResolver] '{skill_name}' (depth={depth}): "
            f"deps={dep_names}"
        )
...
def build_dependency_graph(
    ...
    for skill_name in skill_names:
        ...
        if content is None:
            logger.warning(
                f"[DependencyResolver] build_dependency_graph: "
                f"No content for '{skill_name}', treating as leaf node"
            )
            graph[skill_name] = []
            continue
