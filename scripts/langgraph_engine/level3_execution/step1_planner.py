...
from ..timeout_wrapper import fallback_step1, run_with_timeout
    ...
    def execute(
        ...
        try:
            ...
            try:
                ...
                decision = _llm_call_with_retry(_ollama_step1_call, "Step 1 Plan Mode Decision")
                decision["source"] = "llm"
                ...
                try:
                    ...
                    if claude_client:
                        ...
                        def _claude_step1_call():
                            return claude_client.step1_plan_mode_decision(toon, user_requirement)

                        decision = _llm_call_with_retry(_claude_step1_call, "Step 1 Claude API Fallback")
                        decision["source"] = "claude_api"
                        decision["fallback"] = True
                    else:
                        raise RuntimeError("Claude API client not available")
                except Exception as claude_err:
                    logger.warning(f"Claude API fallback also failed (after retries): {claude_err}")
