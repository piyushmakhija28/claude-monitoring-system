...
from ..plan_convergence import DEFAULT_MAX_ITERATIONS, run_planning_loop
...
from .llm_retry import LLM_BACKOFF_DELAYS, LLM_MAX_RETRIES, is_llm_retryable, llm_call_with_retry
...
class Level3RemainingSteps:
    ...
    def step2_plan_execution(
        self, toon: Dict[str, Any], user_requirement: str, project_root: Optional[str] = None
    ) -> Dict[str, Any]:
        """
...
Be very specific and actionable - mention actual file paths and existing functions/classes."""
    ...
    def _extract_risks(self, plan_text: str) -> Dict[str, Any]:
        ...
        return {"risk_level": risk_level, "factors": factors, "mitigation": mitigation}
    ...
    def step3_task_breakdown(self, plan: str, files_affected: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not valid:
                logger.warning(f"[Step 3] Task breakdown validation found issues: {validation_errors}")
            else:
    ...
    def step4_toon_refinement(
        self, toon_analysis: Dict[str, Any], plan: Dict[str, Any], tasks: List[Dict]
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_effort_estimate(self, complexity_score: int, files_count: int, framework_count: int) -> int:
    ...
    def step6_skill_validation_and_selection(
        self, toon: Dict[str, Any], llm_recommendation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            toon_enhanced = {
                **toon,
                ...
                "available_agents_on_system": [{"name": a["name"]} for a in available_agents],
    ...
    def step13_update_documentation(self, files_modified: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            gen = DocumentationGenerator(project_root=".", session_dir=self.session_manager.session_dir)
    ...
    def step14_final_summary(
        self, issue_number: int, pr_number: int, files_modified: List[str], approach_summary: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            summary = self._build_summary_narrative(issue_number, pr_number, files_modified, approach_summary)
    ...
    def _send_voice_notification(self, summary: str) -> bool:
        ...
        try:
            ...
            if system == "Darwin":  # macOS
                subprocess.run(["say", "Task completed successfully"], timeout=5)
            elif system == "Windows":
                ...
                subprocess.run(["notify-send", "Task", "Completed successfully"], timeout=5)
