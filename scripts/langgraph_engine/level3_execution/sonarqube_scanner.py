...
def get_project_measures(
    ...
    metric_keys = (
        "bugs,vulnerabilities,code_smells,coverage," "duplicated_lines_density,ncloc,sqale_rating,alert_status"
    )
    params: Dict[str, str] = {
        "component": key,
...
def get_quality_gate_status(
    ...
    data = _sonar_api_get("/api/qualitygates/project_status", params=params, config=config)
    if data is None:
        return default
...
def run_basic_scan(
    ...
    if modified_files:
        target_files = [root_path / f for f in modified_files if f.endswith(".py") and (root_path / f).exists()]
    else:
    ...
    target_files = [p for p in target_files if not any(part in _SKIP_DIRS for part in p.parts)]
                    ...
                    findings.append(
                        {
                            ...
                            "message": (f"Function '{node.name}' is {func_len} lines long " f"(threshold: 50)"),
                        }
                    )
