...
Invoked by: level3_execution/steps/step1_plan_mode_decision.py via call_execution_script("auto_plan_mode_suggester", args)
"""
...
class AutoPlanModeSuggester:
    ...
    def should_use_plan_mode(self, complexity_analysis: Dict, structured_prompt: Dict) -> Dict:
        ...
        print(f"\n[WARNING]  Risk Adjustment: +{risks['score']}")
        if risks["factors"]:
            print("   Risk Factors:")
        ...
        adjusted_complexity = self.adjust_complexity_with_risks(complexity_analysis.copy(), risks)
    ...
    def calculate_risk_factors(self, structured_prompt: Dict, complexity: Dict) -> Dict:
        """
        Calculate additional risk factors beyond base complexity
        """
        risks = {"score": 0, "factors": []}
        ...
        if any(kw in prompt_str for kw in ["multiple", "several", "many"]) and any(
            kw in prompt_str for kw in ["fix", "issue", "problem"]
        ):
            risks["score"] += 2
    ...
    def make_decision(self, complexity: Dict) -> Dict:
        ...
        if score < 5:
            ...
            if score >= 8:
                decision["reasoning"] = (
                    "Task leans complex (score 8-9). I recommend plan mode - multiple concerns detected."
                )
            elif score >= 6:
                decision["reasoning"] = (
                    "Task is moderate (score 6-7). I recommend direct proceed - standard patterns apply."
                )
            else:
                decision["reasoning"] = (
                    "Task is low-moderate (score 5). I recommend direct proceed - straightforward execution."
                )
            decision["benefits"] = ["Clearer implementation strategy", "Upfront identification of potential issues"]
            decision["recommendation_text"] = (
                "I recommend plan mode" if score >= 8 else "I recommend proceeding directly"
            )
...
"""
            ...
            for risk in decision["risks_without_planning"]:
                output += f"[WARNING]  {risk}\n"

            output += """
[WARNING] RECOMMENDATION: Enter plan mode (STRONGLY SUGGESTED)
...
def main():
    ...
    if analyze_mode:
        ...
        output = {"plan_required": False, "reasoning": "Task analysis complete", "factors": []}
        ...
        try:
            ...
            complexity = {
                ...
                "level": (
                    ...
                    else "MODERATE" if complexity_score < 10 else "COMPLEX" if complexity_score < 15 else "VERY_COMPLEX"
                ),
            ...
            try:
                ...
                complexity = {"score": 5, "level": "MODERATE", "estimated_tasks": 2, "requires_phases": False}
