...
Invoked by: level3_execution/steps/step5_skill_agent_selection.py via call_execution_script("auto_skill_agent_selector", args)
"""
...
class AutoSkillAgentSelector:
    ...
    def __init__(self):
        ...
        if _agents_dir.exists():
            self.available_agents = sorted(
                [d.name for d in _agents_dir.iterdir() if d.is_dir() and (d / "agent.md").exists()]
            )
        else:
    ...
    def select(self, task_type: str, complexity: Dict, structured_prompt: Dict) -> Dict:
        ...
        selection = {"skills": [], "agents": [], "reasoning": [], "execution_plan": []}
    ...
    def match_technologies(self, technologies: List[str], complexity: int) -> Dict:
        ...
        matches = {"skills": [], "agents": [], "reasoning": []}
        ...
        tech_map = {
            ...
            "java": {"skill": "java-design-patterns-core", "agent": None, "agent_threshold": 999},
            "python": {"skill": "python-core", "agent": "python-backend-engineer", "agent_threshold": 10},
            "flask": {"skill": "python-core", "agent": "python-backend-engineer", "agent_threshold": 10},
            "django": {"skill": "python-core", "agent": "python-backend-engineer", "agent_threshold": 10},
            "langgraph": {"skill": "langgraph-core", "agent": "python-backend-engineer", "agent_threshold": 8},
            "langchain": {"skill": "langchain-core", "agent": "python-backend-engineer", "agent_threshold": 8},
            "docker": {"skill": "docker", "agent": "devops-engineer", "agent_threshold": 12},
            "kubernetes": {"skill": "kubernetes", "agent": "devops-engineer", "agent_threshold": 12},
            "k8s": {"skill": "kubernetes", "agent": "devops-engineer", "agent_threshold": 12},
            "jenkins": {"skill": "jenkins-pipeline", "agent": "devops-engineer", "agent_threshold": 15},
            "postgresql": {"skill": "rdbms-core", "agent": None, "agent_threshold": 999},
            "mysql": {"skill": "rdbms-core", "agent": None, "agent_threshold": 999},
            "mongodb": {"skill": "nosql-core", "agent": None, "agent_threshold": 999},
            "elasticsearch": {"skill": "nosql-core", "agent": None, "agent_threshold": 999},
            "angular": {"skill": None, "agent": "angular-engineer", "agent_threshold": 8},
            "android": {"skill": None, "agent": "android-backend-engineer", "agent_threshold": 10},
        }
        ...
        for tech in technologies:
            for key, config in tech_map.items():
                ...
                if (
                    tech_lower == key
                    or f" {key} " in f" {tech_lower} "
                    or tech_lower.startswith(key + " ")
                    or tech_lower.endswith(" " + key)
                ):
    ...
    def match_task_type(self, task_type: str, complexity: int) -> Dict:
        ...
        if "api" in task_lower or "microservice" in task_lower:
            if "java" in task_lower or "spring" in task_lower:
                ...
                matches["reasoning"].append(
                    f"Task '{task_type}' -> java-spring-boot-microservices skill (always invoke)"
                )
                if any(kw in task_lower for kw in ["multi-step", "full service", "build entire", "architecture"]):
                    ...
                    matches["reasoning"].append(
                        f"Task '{task_type}' -> spring-boot-microservices agent (autonomous workflow)"
                    )
        ...
        if skill_dir.is_dir():
            ...
            if skill_md.exists():
                try:
                    ...
                    skills[name] = {"name": name, "description": description or "Skill module", "has_definition": True}
                except Exception:
                    pass
        ...
        if agent_dir.is_dir():
            ...
            if agent_md.exists():
                try:
                    ...
                    agents[name] = {"name": name, "description": description or "Agent module", "has_definition": True}
                except Exception:
                    pass
        ...
        if project_type and project_type.strip():
            filtered_skills = [
                ...
                or _matches_project(
                    s,
                    (
                        available_skills.get(s, {}).get("description", "")
                        if isinstance(available_skills.get(s), dict)
                        else ""
                    ),
                    project_type,
                )
        ...
        if available_agents:
            if project_type and project_type.strip():
                relevant_agents = {
                    ...
                    or _matches_project(k, v.get("description", "") if isinstance(v, dict) else "", project_type)
                }
            else:
