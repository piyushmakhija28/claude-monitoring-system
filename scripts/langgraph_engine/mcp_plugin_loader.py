    ...
    def load_plugin(self, plugin_name: str) -> MCPPluginInstance:
        ...
        if not module_path.exists():
            raise PluginImplementationError(
                f"Plugin implementation not found: {module_path}"
            )
        ...
        try:
            ...
            if not hasattr(module, manifest.class_name):
                raise PluginImplementationError(
                    f"Class {manifest.class_name} not found in {module_name}"
                )
            ...
            raise PluginImplementationError(
                f"Failed to load {plugin_name}: {e}"
            )
        ...
        try:
            ...
            if not hasattr(plugin, operation):
                raise MCPPluginError(
                    f"Operation {operation} not found in {mcp_name}"
                )
            ...
            raise MCPPluginError(
                f"Failed to route to {mcp_name}.{operation}: {e}"
            )
